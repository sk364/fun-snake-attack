...
def display_game_over_screen(screen, mode):
...
def place_food():
	food.x = random.randrange(0, scr_width, 5)
        food.y = 200#random.randrange(45, scr_height, 5)
...
def game_over(snake,n_blocks,mode):
	...
	if mode == "multi":
		...
		for i in xrange(1,n_blocks):
			if snake[0].x == snake[i].x and snake[0].y == snake[i].y:
				display_game_over_screen(screen, mode)
